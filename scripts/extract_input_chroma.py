...
def _read_slice(
    ...
    """Read a mono slice from a flac. Returns (audio[N], sr).

    Pads with zeros if the file is shorter than start+num. Returns
    (zeros, 0) if file is missing/unreadable.
    """
    try:
...
def _process_window(
    window_dir: Path, project_dir: Path, hpss: bool, overwrite: bool
) -> str:
    out_path = window_dir / "input_chroma.pt"
    ...
    chroma = compute_chroma_from_audio(
        mix,
...
def main():
    ...
    for split in args.splits:
        ...
        for w in tqdm(windows, desc=f"{split}", mininterval=10.0):
            result = _process_window(w, project_dir, args.hpss, args.overwrite)
            counts[result] += 1
            total_processed += 1
