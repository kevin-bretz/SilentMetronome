...
from stream_music_gen.lit_module.dec_online import (
    LitDecoderOnline,
)
...
def generate_prediction(
    batch: any,
    model: DecoderTransformerMultiOut,
    ...
    original_targets = batch["target_token"].to(device)
    targets = original_targets
    dec_inputs_mask = batch["target_token_mask"].to(device)

    if model.future_visibility > 0:
        # pad targets
        target_pad = torch.full(
            (*targets.shape[:2], model.future_visibility - 1),
            tokenizer.pad_token,
            dtype=targets.dtype,
            device=targets.device,
        )
        targets = torch.cat(
            (target_pad, targets[:, :, : -model.future_visibility + 1]), dim=2
        )
    ...
    if model.future_visibility <= 0:
        ...
        mask_padding = torch.ones(
            (dec_inputs_mask.shape[0], model.future_visibility - 1),
        ...
        dec_inputs_mask = torch.cat(
            (
                mask_padding,
                dec_inputs_mask[:, : -model.future_visibility + 1],
            ),
            dim=1,
        )

    dec_inst_tokens = torch.tensor(
        batch["target_inst_token"], device=targets.device
    )

    # FIX: Extract prompt from the padded targets, not the original target_token
    if prompt_secs > 0:
        prompt_frames = prompt_secs * tokenizer.frame_rate
        if model.future_visibility > 0:
            # For positive future_visibility, prompt should be extracted from padded targets
            # The padding size is (model.future_visibility - 1), so the original sequence starts after that
            padding_size = model.future_visibility - 1
            seq_out_start = targets[
                :, :, padding_size : padding_size + prompt_frames
            ]
        else:
            # For non-positive future_visibility, use original logic
            seq_out_start = batch["target_token"][:, :, :prompt_frames].to(
                device
            )
    else:
        seq_out_start = None
    ...
    for i in range(targets.size(0)):
        ...
        # Get Target Audio (use original un-padded tokens, not the
        # future-visibility-padded `targets` which has pad tokens that
        # post_process_tokens would strip, producing shorter audio)
        target_audio_tokens = tokenizer.post_process_tokens(original_targets[i])
        target_audio_single = (
...
def main(
    ...
    model, tokenizer, dataloaders = load_lit_model(
        model_path,
        lit_module_cls=LitDecoderOnline,
