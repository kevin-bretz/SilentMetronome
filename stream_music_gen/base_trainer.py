...
class Trainer:
    ...
    def __init__(
        ...
        wandb_id = None
        if self.ckpt_path:
            wandb_root = self.save_dir / "wandb"
            if wandb_root.exists():
                # latest-run can point at a dead run that never wrote its
                # .wandb file (wandb daemon startup failure), so scan all
                # run-*/run-*.wandb files and pick the most recent.
                wandb_files = sorted(
                    wandb_root.glob("run-*/run-*.wandb"),
                    key=lambda p: p.stat().st_mtime,
                )
                if wandb_files:
                    wandb_id = wandb_files[-1].stem.split("-")[-1]
