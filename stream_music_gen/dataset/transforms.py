...
from stream_music_gen.dataset.beat_grid_utils import derive_track_name
...
class SelectStems(BaseTransform):
    ...
    def load_stems(self, item, input_idxs, target_idx, start):
        ...
        # -- Window timing (needed for beat-phase conditioning preprocessing)
        output_item["start_frame"] = int(start)
        output_item["start_time_sec"] = float(start) / float(self.rms_token_hz)

        # -- Track name (parsed from target token path, e.g. "Track00305" in slakh;
        #    None for datasets without this convention). Always set so collate
        #    keys stay consistent across items.
        output_item["track_name"] = derive_track_name(
            output_item["target_token_path"]
        )
