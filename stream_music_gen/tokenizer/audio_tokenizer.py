...
class EncodecAudioTokenizer(BaseAudioTokenizer):
    ...
    def tokens_to_audio(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        # codec_tokens: (b, n, t)
        # Degenerate rollouts (all special tokens stripped by post_process_tokens)
        # produce empty codec_tokens. Return an empty audio tensor so the caller
        # can decide how to handle it instead of crashing on .max() of empty.
        if codec_tokens.numel() == 0:
            return torch.zeros(0, device=codec_tokens.device)
        if (
